...
int main() {
  ...
  auto sketchCamera =
      ENDER::OrthographicCamera::create({0, 5, 0}, {SCR_WIDTH, SCR_HEIGHT});
  ...
  while (!ENDER::Window::windowShouldClose() && p_open) {
    ENDER::Renderer::begin([&]() {
      ...
      if (ENDER::Window::isMouseButtonPressed(
              ENDER::Window::MouseButton::Left) &&
          ImGui::IsWindowFocused()) {
        ImVec2 screen_pos = ImGui::GetCursorScreenPos();
        auto mousePosition = ENDER::Window::getMousePosition();
        auto pickedID = framebuffer->pickObjAt(
      ...
      ImGui::Image((ImTextureID)framebuffer->getTextureId(),
