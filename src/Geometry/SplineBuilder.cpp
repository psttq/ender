...
void SplineBuilder::addPoint(sptr<Point> point) { points.push_back(point); }

void SplineBuilder::removePoint(sptr<Point> point) {
  auto iter = std::remove(points.begin(), points.end(), point);
  points.erase(iter, points.end());
}

std::vector<sptr<Point>> SplineBuilder::getPoints() { return points; }
...
sptr<Point> LinearInterpolationBuilder::_getSplineFirstDerivative(float t) {
  uint j = 0;
  while (t > _t[j + 1])
    j++;
  ...
  glm::vec3 firstDerivative;
  firstDerivative.x =
      (points[j + 1]->getPosition().x - points[j]->getPosition().x) / h;
  firstDerivative.y =
      (points[j + 1]->getPosition().y - points[j]->getPosition().y) / h;
  firstDerivative.z =
      (points[j + 1]->getPosition().z - points[j]->getPosition().z) / h;

  return Point::create(firstDerivative);
}

sptr<Point> LinearInterpolationBuilder::_getSplineSecondDerivative(float t) {
  // The second derivative of a linear spline is always zero.
  return Point::create(glm::vec3(0.0f, 0.0f, 0.0f));
}

std::vector<sptr<Point>>
LinearInterpolationBuilder::getSplineDerivatives(float t, int dirsCount) {
  return {getSplinePoint(t), _getSplineFirstDerivative(t),
          _getSplineSecondDerivative(t)};
}

sptr<Point> LinearInterpolationBuilder::_getSplinePoint(float t) {
  uint j = 0;
  while (t > _t[j + 1])
    j++;
...
CubicSplineBuilder::CubicSplineBuilder(const std::vector<sptr<Point>> &points)
    : SplineBuilder(points) {
  for (auto point : points) {
    auto pos = point->getPosition();
    x.push_back(pos.x);
    y.push_back(pos.z);

    x_min = std::min(pos.x, x_min);
    x_max = std::max(pos.x, x_max);
  }
  n = points.size();
};
...
uptr<SplineBuilder> CubicSplineBuilder::clone() {
  ...
  return uptr<CubicSplineBuilder>(new CubicSplineBuilder(pointsCopy));
}

sptr<Point> CubicSplineBuilder::_getSplineFirstDerivative(float t) {
  t = x_min + (x_max - x_min) * t;
  for (size_t i = 0; i < n - 1; ++i) {
    if (x[i] <= t && t <= x[i + 1]) {
      float dx = t - x[i];
      float derivative = coefficients[i].b + 2.0f * coefficients[i].c * dx +
                         3.0f * coefficients[i].d * dx * dx;
      return Point::create({t, 0, derivative});
    }
  }
...
sptr<Point> CubicSplineBuilder::_getSplineSecondDerivative(float t) {
  t = x_min + (x_max - x_min) * t;
  for (size_t i = 0; i < n - 1; ++i) {
    if (x[i] <= t && t <= x[i + 1]) {
      float dx = t - x[i];
      float second_derivative =
          2.0f * coefficients[i].c + 6.0f * coefficients[i].d * dx;
...
std::vector<sptr<Point>>
CubicSplineBuilder::getSplineDerivatives(float t, int dirsCount) {
  return {getSplinePoint(t), _getSplineFirstDerivative(t),
          _getSplineSecondDerivative(t)};
}

void CubicSplineBuilder::rebuild() {
  ...
  for (size_t j = n - 2; j < n - 1; --j) {
    ...
    coefficients[j].b =
        (y[j + 1] - y[j]) / h[j] -
        h[j] * (coefficients[j + 1].c + 2.0f * coefficients[j].c) / 3.0f;
    coefficients[j].d =
        (coefficients[j + 1].c - coefficients[j].c) / (3.0f * h[j]);
    coefficients[j].a = y[j];
  }
}

sptr<Point> CubicSplineBuilder::_getSplinePoint(float t) {
  t = x_min + (x_max - x_min) * t;
  for (size_t i = 0; i < n - 1; ++i) {
    if (x[i] <= t && t <= x[i + 1]) {
      float dx = t - x[i];
...
bool CubicSplineBuilder::drawPropertiesGui() {
  for (auto &coef : coefficients) {
    ImGui::Text("a: %f, b: %f, c: %f, d: %f", coef.a, coef.b, coef.c, coef.d);
  }
...
sptr<Point> BezierBuilder::_getSplinePoint(float t) {
  if (useDeCasteljau) {
...
sptr<Point> RationalBezierBuilder::_getSplinePoint(float t) {
...
sptr<Point> BSplineBuilder::_getSplinePoint(float t) {
...
sptr<Point> RationalBSplineBuilder::_getSplinePoint(float t) {
