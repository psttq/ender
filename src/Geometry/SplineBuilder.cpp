...
namespace EGEOM
{
    ...
    for (size_t i = 0; i < n - 1; ++i)
    {
      if (x[i] <= t && t <= x[i + 1])
      {
        ...
        float second_derivative =
            2.0f * coefficients[i].c +
            6.0f * coefficients[i].d * dx;
  ...
  std::vector<sptr<Point>> CubicSplineBuilder::getSplineDerivatives(float t,
                                                                    int dirsCount)
  {
    return {getSplinePoint(t), _getSplineFirstDerivative(t), _getSplineSecondDerivative(t)};
  }
