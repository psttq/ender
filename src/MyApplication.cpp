...
void MyApplication::handleDebugGUI() {
    ...
    if (ImGui::SliderInt("Interpolation Points Count", &interpolationPointsCount,
                         2, 300)) {
        sketches[currentSketchId]->getSpline()->setInterpolationPointsCount(interpolationPointsCount);
...
void MyApplication::render() {
    ...
    if (renderDebugSplinePoints)
        for (auto p: sketches[currentSketchId]->getSpline()->getInterpolatedPoints()) {
    ...
    ENDER::Renderer::renderObject( sketches[currentSketchId]->getSpline(), sketchScene, sketchFramebuffer);
}
...
void MyApplication::handleSketchSideGUI() {
    ...
    if (ImGui::Button("Add sketch")) {
        auto spline = EGEOM::Spline1::create({}, EGEOM::Spline1::ParamMethod::Uniform, interpolationPointsCount);
        auto newSketch = EGEOM::Sketch::create("Sketch " + std::to_string(sketches.size()), spline);
    ...
    std::for_each(sketches.begin(), sketches.end(),
                  [&sketchesNameList](auto sketch) { sketchesNameList.push_back(sketch->name.c_str()); });
    ImGui::ListBox("Sketches List", &currentSketchId, &sketchesNameList[0], sketchesNameList.size(), 4);
    ImGui::End();
}
...
void MyApplication::handleObjectsGUI() {
    ...
    std::for_each(viewportScene->getObjects().begin(), viewportScene->getObjects().end(),
                  [&objectsNameList](auto object) { objectsNameList.push_back(object->getName().c_str()); });
    static int currentObject = 0;
    ImGui::ListBox("Objects List", &currentObject, &objectsNameList[0], objectsNameList.size(), 4);
    ImGui::End();
}
