...
void MyApplication::onStart() {
  ...
  planeVBO->setData(ENDER::debugSquareVertices,
                    sizeof(ENDER::debugSquareVertices));

  auto planeIBO =
      std::make_unique<ENDER::IndexBuffer>(ENDER::debugSquareIndices, 6);
  auto planeVAO = std::make_shared<ENDER::VertexArray>();
  planeVAO->addVBO(std::move(planeVBO));
  planeVAO->setIndexBuffer(std::move(planeIBO));

  auto planeObject = ENDER::Object::create("Plane", planeVAO);
  planeObject->isSelectable = true;

  viewportScene->addObject(planeObject);
