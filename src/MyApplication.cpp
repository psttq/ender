...
//    config.GlyphMinAdvanceX = 13.0f; // Use if you want to make the icon monospaced
    ...
    io.Fonts->AddFontFromFileTTF("../resources/fa-solid-900.ttf", 13.0f, &config, icon_ranges);
...
//        sketchScene->addObject(line);
    ...
    spline1 = std::make_shared<EGEOM::Spline1>(points, EGEOM::Spline1::ParamMethod::Uniform, interpolationPointsCount);
    spline1->material.ambient = {0.2, 0.5, 0.2};
    spline1->material.diffuse = {0.2, 0.5, 0.2};

    sketchScene->addObject(spline1);
}
...
void MyApplication::handleDebugGUI() {
    ...
    if (ImGui::SliderInt("Interpolation Points Count", &interpolationPointsCount, 2, 300)) {
        spline1->setInterpolationPointsCount(interpolationPointsCount);
        for (auto p: spline1->getInterpolatedPoints()) {
            sketchScene->deleteObject(p);
        }
        spline1->update();
        for (auto p: spline1->getInterpolatedPoints()) {
            p->material.ambient = {0.0, 0.6, 0.6};
            p->material.diffuse = {0.0, 0.6, 0.6};
            sketchScene->addObject(p);
        }
    }
...
void MyApplication::onMouseClick(ENDER::Window::MouseButton button, ENDER::Window::EventStatus status) {
