    ...
    if (ImGui::Button("Create")) {
      ...
      if (pivot && pivot->getSketch()) {
        ...
        auto bottom = EGEOM::PlaneSurface::create(
            wire->getPoint(0), wire->getPoint(0.2), wire->getPoint(0.7));
        ...
        for (auto edge : edges) {
          ...
          auto nurbsBuilder = uptr<EGEOM::RationalBSplineBuilder>(
              new EGEOM::RationalBSplineBuilder(sideSpline->getPoints(), 1, {},
                                                {}));
        ...
        auto upper = EGEOM::PlaneSurface::create(upperWire->getPoint(0.1),
