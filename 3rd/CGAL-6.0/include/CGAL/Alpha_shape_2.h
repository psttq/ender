...
//
// $URL:
// https://github.com/CGAL/cgal/blob/v6.0/Alpha_shapes_2/include/CGAL/Alpha_shape_2.h
// $ $Id: include/CGAL/Alpha_shape_2.h 50219fc33bc $ Author(s)     : Tran Kai
// Frank DA
...
public:
  ...
  // The Exact Comparison Tag cannot be used in conjunction with periodic
  // triangulations because the periodic triangulations' point() function
  // returns a temporary value while the lazy predicate evaluations that are
  // used when the Exact tag is set to true rely on a permanent and safe access
  // to the points.
  static_assert((std::is_same<ExactAlphaComparisonTag, Tag_false>::value) ||
                (std::is_same<typename Dt::Periodic_tag, Tag_false>::value));

  typedef typename internal::Alpha_nt_selector_2<
      Gt, ExactAlphaComparisonTag, typename Dt::Weighted_tag>::Type_of_alpha
      Type_of_alpha;
  typedef typename internal::Alpha_nt_selector_2<Gt, ExactAlphaComparisonTag,
                                                 typename Dt::Weighted_tag>::
      Compute_squared_radius_2 Compute_squared_radius_2;
  typedef typename internal::Alpha_nt_selector_2<Gt, ExactAlphaComparisonTag,
                                                 typename Dt::Weighted_tag>::
      Side_of_bounded_circle_2 Side_of_bounded_circle_2;
...
public:
  typedef
      typename std::list<Vertex_handle>::iterator Alpha_shape_vertices_iterator;
...
public:
  ...
  Alpha_shape_2(Type_of_alpha alpha = Type_of_alpha(0), Mode m = GENERAL)
  ...
  Alpha_shape_2(const InputIterator &first, const InputIterator &last,
                const Type_of_alpha &alpha = Type_of_alpha(0), Mode m = GENERAL)
      : _alpha(alpha), _mode(m), Infinity(-1), UNDEFINED(-2),
        use_vertex_cache(false), use_edge_cache(false) {
  ...
  Alpha_shape_2(Dt &dt, const Type_of_alpha &alpha = Type_of_alpha(0),
                Mode m = GENERAL)
      : _alpha(alpha), _mode(m), Infinity(-1), UNDEFINED(-2),
        use_vertex_cache(false), use_edge_cache(false) {
    Dt::swap(dt);
...
private:
  struct Less {
    bool operator()(const Interval_edge &ie, const Type_of_alpha &alpha) {
      return ie.first.first < alpha;
    }

    bool operator()(const Type_of_alpha &alpha, const Interval_edge &ie) {
      return alpha < ie.first.first;
    }

    // Needed for STL implementations of upper_bound which in debug mode
    // check sortedness of range
    bool operator()(const Interval_edge &ie, const Interval_edge &ie2) const {
      return ie < ie2;
    }
  };
...
private:
  ...
  Interval3 find_interval(const_Edge e) const { // corriger le parametrage
...
private:
  ...
  void update_alpha_shape_edges_list() const;
...
public:
  ...
  Alpha_iterator alpha_find(const Type_of_alpha &alpha) const {
    ...
    return std::find(_alpha_spectrum.begin(), _alpha_spectrum.end(), alpha);
  }

  Alpha_iterator alpha_lower_bound(const Type_of_alpha &alpha) const {
    // Returns an iterator pointing to the first element with
    // alpha-value not less than `alpha'.
    return std::lower_bound(_alpha_spectrum.begin(), _alpha_spectrum.end(),
                            alpha);
  }
  ...
  Alpha_iterator alpha_upper_bound(const Type_of_alpha &alpha) const {
    ...
    return std::upper_bound(_alpha_spectrum.begin(), _alpha_spectrum.end(),
                            alpha);
  }
  ...
  Classification_type classify(const Point &p,
    ...
    switch (type) {
    case VERTEX:
      return classify(pFace->vertex(i), alpha);
    case EDGE:
      return classify(pFace, i, alpha);
    case FACE:
      return classify(pFace, alpha);
  ...
  Classification_type classify(const Face_handle &f, int i) const {
  ...
  Classification_type classify(const Face_handle &f, int i,
                               const Type_of_alpha &alpha) const;
  ...
  //--------------------- NB COMPONENTS ---------------------------------
  size_type number_solid_components() const {
  ...
  size_type number_solid_components(const Type_of_alpha & /* alpha */) const {
    return number_of_solid_components(get_alpha());
  }

  size_type number_of_solid_components(const Type_of_alpha &alpha) const;

private:
  void traverse(const Face_handle &pFace, Marked_face_set &marked_face_set,
                const Type_of_alpha alpha) const;
...
private:
  bool is_attached(const Face_handle &f, int i) const {
    Bounded_side b = Side_of_bounded_circle_2()(*this)(
        point(f, cw(i)), point(f, ccw(i)), point(f, i));
  ...
  Type_of_alpha squared_radius(const Face_handle &f) const {
    return Compute_squared_radius_2()(*this)(point(f, 0), point(f, 1),
                                             point(f, 2));
  }

  Type_of_alpha squared_radius(const Face_handle &f, int i) const {
    return Compute_squared_radius_2()(*this)(point(f, ccw(i)), point(f, cw(i)));
  }
...
void Alpha_shape_2<Dt, EACT>::initialize_interval_face_map() {
  ...
  // only finite faces
  for (Finite_faces_iterator face_it = faces_begin(); face_it != faces_end();
       ++face_it) {
...
void Alpha_shape_2<Dt, EACT>::initialize_interval_edge_map() {
  ...
  for (edge_it = edges_begin(); edge_it != edges_end(); ++edge_it) {
    ...
    if (!is_infinite(pFace) && !is_infinite(pNeighbor)) {
      ...
      interval =
          (is_attached(pFace, i) || is_attached(pNeighbor, Neigh_i))
              ? make_triple(UNDEFINED, squared_radius_Face,
      ...
      if (is_infinite(pFace)) {
        if (!is_infinite(pNeighbor)) {
          ...
          CGAL_precondition((is_infinite(pNeighbor) && is_infinite(pFace)));
          interval = make_triple(squared_radius(pFace, i), Infinity, Infinity);
        }
      } else { // is_infinite(pNeighbor)

        CGAL_precondition((is_infinite(pNeighbor) && !is_infinite(pFace)));
        if (is_attached(pFace, i))
          interval = make_triple(UNDEFINED, find_interval(pFace), Infinity);
        else
          interval = make_triple(squared_radius(pFace, i), find_interval(pFace),
                                 Infinity);
      }
    }
...
template <class Dt, class EACT>
void Alpha_shape_2<Dt, EACT>::initialize_interval_vertex_map() {
  ...
  for (vertex_it = finite_vertices_begin(); vertex_it != finite_vertices_end();
       ++vertex_it) {
    ...
    alpha_mid_v =
        (!_interval_face_map.empty() ? (--_interval_face_map.end())->first
                                     : Type_of_alpha(0));
    ...
    Face_circulator face_circ = this->incident_faces(v), done = face_circ;
...
template <class Dt, class EACT>
void Alpha_shape_2<Dt, EACT>::initialize_alpha_spectrum() {

  // skip the attached edges
  // <=> _interval_edge_map.first.first == UNDEFINED
  typename Interval_edge_map::iterator edge_it = std::upper_bound(
      _interval_edge_map.begin(), _interval_edge_map.end(), UNDEFINED, Less());
  ...
  typename Interval_face_map::iterator face_it = _interval_face_map.begin();
...
template <class Dt, class EACT>
void Alpha_shape_2<Dt, EACT>::update_alpha_shape_vertex_list() const {
  ...
  for (vertex_alpha_it = _interval_vertex_map.begin();
    ...
    if ((pInterval2->second > get_alpha() || pInterval2->second == Infinity)) {
  ...
  if (get_mode() == Alpha_shape_2<Dt, EACT>::GENERAL) {
    ...
    for (; vertex_alpha_it != _interval_vertex_map.end(); ++vertex_alpha_it) {
      v = (*vertex_alpha_it).second;
...
template <class Dt, class EACT>
void Alpha_shape_2<Dt, EACT>::update_alpha_shape_edges_list() const {
  ...
  if (get_mode() == REGULARIZED) {
    ...
    for (edge_alpha_it = _interval_edge_map.begin();
      ...
      if (pInterval->second <= get_alpha() &&
          (pInterval->third > get_alpha() || pInterval->third == Infinity)) {
        // alpha must be larger than the mid boundary
        // and alpha is smaller than the upper boundary
        // which might be infinity
    ...
    for (edge_alpha_it = _interval_edge_map.begin();
      ...
      if (pInterval->first == UNDEFINED) {
        ...
        if (pInterval->second <= get_alpha() &&
          ...
          CGAL_assertion((classify((*edge_alpha_it).second.first,
                                   (*edge_alpha_it).second.second) == REGULAR));
          Alpha_shape_edges_list.push_back(Edge(
              (*edge_alpha_it).second.first, (*edge_alpha_it).second.second));
        }
      } else {

        if (pInterval->third > get_alpha() || pInterval->third == Infinity) {
...
Alpha_shape_2<Dt, EACT>::classify(const Face_handle &f, int i,
  ...
  if (alpha < interval.second) {
    if (get_mode() == REGULARIZED || interval.first == UNDEFINED ||
...
Alpha_shape_2<Dt, EACT>::classify(const Vertex_handle &v,
  ...
  if (alpha < interval.first) {
    ...
    if (interval.second == Infinity || alpha < interval.second)
      return REGULAR;
    else // alpha >= interval.second
      return INTERIOR;
  }
...
Alpha_shape_2<Dt, EACT>::number_of_solid_components(
    const Type_of_alpha &alpha) const {
...
template <class Dt, class EACT>
void Alpha_shape_2<Dt, EACT>::traverse(const Face_handle &pFace,
...
Alpha_shape_2<Dt, EACT>::find_alpha_solid() const {
  ...
  for (vertex_it = finite_vertices_begin(); vertex_it != finite_vertices_end();
    ...
    do {
      ...
      if (!is_infinite(f))
        alpha_min_v = (CGAL::min)(find_interval(f), alpha_min_v);
    } while (++face_circ != done);
...
template <class Dt, class EACT>
std::ostream &Alpha_shape_2<Dt, EACT>::op_ostream(std::ostream &os) const {
  typedef
      typename Alpha_shape_2<Dt, EACT>::Interval_vertex_map Interval_vertex_map;
  ...
  if (get_mode() == Alpha_shape_2<Dt, EACT>::REGULARIZED) {
...
#ifdef CGAL_DEBUG_ALPHA_SHAPE_2
      typename Alpha_shape_2<Dt, EACT>::Type_of_alpha alpha = get_alpha();
    ...
    for (edge_alpha_it = _interval_edge_map.begin();
      ...
      if (pInterval->second <= get_alpha() &&
        ...
        if (classify(f) == Alpha_shape_2<Dt, EACT>::EXTERIOR) {
          ...
          typename Alpha_shape_2<Dt, EACT>::Face_handle pNeighbor =
              f->neighbor(i);
          i = pNeighbor->index(f);
          f = pNeighbor;
        }

        CGAL_assertion((classify(f) == Alpha_shape_2<Dt, EACT>::INTERIOR));

        CGAL_assertion((classify(f, i) == Alpha_shape_2<Dt, EACT>::REGULAR));

        os << V[f->vertex(f->ccw(i))] << ' ' << V[f->vertex(f->cw(i))]
           << std::endl;
      }
    }
  } else { // get_mode() == GENERAL -----------------------------------------
    ...
    for (edge_alpha_it = _interval_edge_map.begin();
      ...
      if (pInterval->third > get_alpha() || pInterval->third == Infinity) {
        ...
        if (pInterval->second != Infinity && pInterval->second <= get_alpha()) {
          CGAL_assertion((classify(f, i) == Alpha_shape_2<Dt, EACT>::REGULAR));
          // assure that all vertices are in ccw order
          if (classify(f) == Alpha_shape_2<Dt, EACT>::EXTERIOR) {
            // take the reverse face
          ...
          os << V[f->vertex(f->ccw(i))] << ' ' << V[f->vertex(f->cw(i))]
             << std::endl;
        } else { // pInterval->second == Infinity ||
          ...
          if (pInterval->first != UNDEFINED) {
            CGAL_assertion(
                (classify(f, i) == Alpha_shape_2<Dt, EACT>::SINGULAR));
            os << V[f->vertex(f->ccw(i))] << ' ' << V[f->vertex(f->cw(i))]
               << std::endl;
          }
        }
...
template <class Dt, class EACT>
std::ostream &operator<<(std::ostream &os, const Alpha_shape_2<Dt> &A) {
...
template <class Dt, class EACT> void Alpha_shape_2<Dt, EACT>::print_edge_map() {
  for (typename Interval_edge_map::iterator iemapit =
           _interval_edge_map.begin();
