...
class BSplineBuilder : public SplineBuilder {
  ...
  void _checkAndSetDefault() {
    if (points.size() < bSplinePower + 1) {
      spdlog::warn(
          "BSpline: When creating BSplineBuilder, not enough controls "
    ...
    if (knotVector.size() != points.size() + bSplinePower + 1) {
      ...
      this->knotVector =
          std::vector<float>(points.size() + bSplinePower + 1, 0);
    ...
    if (ImGui::TreeNode("Knot Vector")) {
      ...
      if (child_is_visible) {
        for (auto i = 0; i < knotVector.size(); i++) {
          ...
          if (i < bSplinePower + 1 ||
              i >= knotVector.size() - bSplinePower - 1) {
            ImGui::BeginDisabled();
...
class RationalBSplineBuilder : public SplineBuilder {
  ...
  void _checkAndSetDefault() {
    if (points.size() < bSplinePower + 1) {
      spdlog::warn(
          "RationalBSpline: When creating BSplineBuilder, not enough controls "
          "points(len = {}) to create BSpline(power = {}). Should be "
          "len(points) >= degree + 1. Changing power to len(points)-1. ",
          points.size(), bSplinePower);
      bSplinePower = points.size() - 1;
    ...
    if (knotVector.size() != points.size() + bSplinePower + 1) {
      spdlog::warn(
          ...
          "enough elements. Should be len(points)+bSplinePower+1. Initializing "
          "with default one.");
      this->knotVector =
          std::vector<float>(points.size() + bSplinePower + 1, 0);
      for (auto i = 0; i < bSplinePower + 1; i++) {
